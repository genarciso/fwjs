<form class="qst-form" on:submit|preventDefault={submit}>
    <div>
        ...
        <textarea id="stat" bind:value={statement} on:change={changeStatement}></textarea>
        {#if errors.stat }
    ...
    {#if qstType === 'choice'}
        ...
        <input id="numOptions" type="number" min="2" max="5" value={options.length} on:change={e => changeNumOptions(e.target['value'])} />
        ...
        {#each options as opt, i (`option_${i}`)}
            <div>
                ...
                <input id="option" type="text" bind:value={options[i]} on:change={e => changeOption(i, e.target['value'])} />
                {#if errors[`option_${i}`] }
